# Claude Code Project Instructions — keyring-kotlin

## Project Overview

This is **keyring-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`keyring`](https://crates.io/crates/keyring).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/keyring/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/keyring>
